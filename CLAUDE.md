...
### Portable, zero-install builds (`packaging/`)

`release.yml`'s `build-windows` job produces `pdf-ocr-corrector-windows-portable.zip` and attaches it to
the GitHub Release: a `pyinstaller --onedir` build of `app.py` with Tesseract vendored inside it, so
end users need nothing pre-installed - not Python, not Tesseract. `--onedir` (a folder) rather than
`--onefile` (a self-extracting exe) is deliberate: `--onefile` re-extracts the whole bundle to a fresh
temp directory on *every* launch, which is slow and wasteful once Tesseract's language data is in the
mix (~100MB). `--onedir` just sits there; "portable" here means "zip the folder, move it, run the exe
inside it" rather than "single file."

`packaging/vendor_tesseract_windows.sh` builds the vendored Tesseract folder from two pinned sources
rather than trusting whatever's currently latest: it extracts `tesseract.exe` and its DLLs directly out
of the official NSIS installer *without running it* (`7z x installer.exe` - NSIS installers are
self-extracting 7z archives, no need for a GUI install just to get the files), then separately downloads
`eng.traineddata`/`osd.traineddata` from `tesseract-ocr/tessdata_fast` pinned to a commit. Language data
isn't in the installer archive itself - the installer fetches it from the network during setup - which
is why it needs a second source.

`app.py` splits `BASE_DIR` (where the executable lives - used for `work/`, which must persist across
runs and sit next to the exe, not in a temp extraction dir) from `RESOURCE_DIR` (`sys._MEIPASS` when
frozen - where bundled read-only assets like `static/`, `VERSION`, and the vendored Tesseract were
unpacked to) at import time; see the comment above `BASE_DIR` in `app.py`. `_find_tesseract()` checks
`RESOURCE_DIR/tesseract/tesseract.exe` right after the `TESSERACT_CMD` override, before falling back to
`PATH`/known install locations - the bundled copy should win over a stray system install by default,
since it's the version this build was actually tested against.

The `packaging/pdf-ocr-corrector.spec` file expects vendored assets at `packaging/vendor/<platform>/
tesseract/` (lowercase `platform.system()`) - `windows/` exists and is CI-verified; `darwin/`/`linux/`
don't have a vendoring script yet (see `TODO.md`). Unlike Windows, Tesseract on macOS/Linux is normally
dynamically linked against system libraries (Homebrew's `leptonica`, etc.), not a self-contained folder,
so vendoring it means collecting and rpath-fixing every shared library dependency (`dylibbundler` /
`patchelf`+`ldd`-style tooling) rather than just extracting an installer - a different, harder problem
per OS, not a copy-paste of the Windows script.

`packaging/vendor/`, `packaging/dist/`, and `packaging/build/` are gitignored - the vendored binaries and
build output are regenerated by CI (or `bash packaging/vendor_tesseract_windows.sh <dir>` locally) and
